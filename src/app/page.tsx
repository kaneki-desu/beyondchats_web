...
const apiURL: string = process.env.NEXT_PUBLIC_BEYONDCHAT_API!;
const mockResponse1 = {
...
export default function Home() {
  ...
  function extractPersona(raw: unknown): any {
    try {
      if (typeof raw === 'object' && raw !== null) {
        const parsed = raw as { persona?: unknown };
      ...
      const personaResponse = parsed as { persona?: unknown };

      return personaResponse.persona || parsed;
    } catch (err) {
      console.error('❌ Failed to parse persona:', err);
      if (err instanceof SyntaxError && /Unexpected end/i.test(err.message)) {
        throw new Error('The persona API returned incomplete JSON. Please try again.', { cause: err });
      }
      throw err;
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!res.ok) {
      throw new Error(`Persona API request failed with status ${res.status}.`);
    }
